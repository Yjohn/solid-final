...
export type Role = "patient" | "doctor" | "emergency" | "pharmacy" | "nurse" | "unknown";
...
const App: React.FC = () => {
  ...
  // Initialise Solid session on page load
  useEffect(() => {
    (async () => {
  ...
  // Whenever the selected patient or role changes, load that patient's record and files
  useEffect(() => {
    if (!loggedIn) {
      setFullRecord(null);
    ...
    const effectivePatient = getEffectivePatientKey(role, webId, selectedPatient);
    if (!effectivePatient) {
      setFullRecord(null);
      setFullRecordStatus(null);
      setFullRecordError("Could not determine which patient this WebID belongs to.");
      setPatientFiles([]);
      return;
    }
  ...
  function getEffectivePatientKey(
    ...
    // Doctor, emergency, pharmacy, nurse, unknown -> use dropdown selection
    return selected;
  }
  ...
  async function handleFileUpload(fileData: Omit<PatientFile, "id" | "createdAt" | "updatedAt">) {
    const effectivePatient = getEffectivePatientKey(role, webId, selectedPatient);
    if (!effectivePatient) {
      alert("No patient selected.");
      return;
  ...
  function renderPatientSelector() {
    ...
    // Doctor, emergency, pharmacy, nurse
    const keys = Object.keys(PATIENTS) as PatientKey[];
    ...
    if (fullRecordStatus === 403) {
      return (
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 mb-6">
          <h2 className="text-xl font-bold text-gray-800 mb-2">Full record</h2>
          <p className="text-red-700">You do not have access to this patient's full record.</p>
        </div>
      );
    }

    if (fullRecordStatus === 404) {
      // For patient and doctor, show empty form. For others, just say "no record yet".
  ...
  function renderAccessControls() {
    ...
    // Only the patient can change access for their record
    const effectivePatient = getEffectivePatientKey(role, webId, selectedPatient);
    const isOwner =
      role === "patient" &&
      effectivePatient &&
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-6">
        ...
        <p className="text-gray-600 mb-4">
          ...
          and the emergency contact. These settings are written to an ACP ACR document
          in your Solid pod.
        </p>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
              ...
              {role === "doctor" && (
                <p className="text-gray-600">
                  ...
                  You can also add and manage patient files.
                </p>
              )}
              {role === "emergency" && (
                <p className="text-gray-600">
                  As the emergency profile, you should only see records where the
                  ACP grants you read access. If there is no access you will see a
                  clear message instead of another patient's data.
                </p>
              )}
              {role === "patient" && (
                <p className="text-gray-600">
                  You are logged in as the patient. You can add lab results, prescriptions,
                  and other medical files to your record. Use the access control card to
                  grant or revoke access for other roles.
                </p>
              )}
              {role === "pharmacy" && (
                <p className="text-gray-600">
                  As a pharmacy, you can view prescription files that have been shared with you.
                </p>
              )}
              {role === "nurse" && (
                <p className="text-gray-600">
                  As a nurse, you can view patient files and assist with data entry under
                  doctor supervision.
                </p>
              )}
              {role === "unknown" && (
...
// FullRecordForm component (same as before, but kept here for completeness)
...
const FullRecordForm: React.FC<FullRecordFormProps> = ({
  ...
  // Only emergency, pharmacy, nurse, and unknown roles should be read-only
  // Patients and doctors can edit
  const readOnly = role === "emergency" || role === "pharmacy" || role === "nurse" || role === "unknown";
